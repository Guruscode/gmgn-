...
interface HolderInsights {
  ...
  holderChange?: Record<string, HolderChange>;
  holderSupply?: Record<string, HolderSupply>;
}

interface TokenHolderInsightsProps {
  token: Token | null;
  chainId: string;
}
...
const TokenHolderInsights: React.FC<TokenHolderInsightsProps> = ({ token, chainId }) => {
  const [insights, setInsights] = useState<HolderInsights | null>(null);
  ...
  const prepareAcquisitionData = (): ChartData<'doughnut'> => {
